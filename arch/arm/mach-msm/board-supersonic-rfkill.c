...
static int supersonic_rfkill_probe(struct platform_device *pdev)
{
	...
	rc = gpio_request(SUPERSONIC_GPIO_BT_RESET_N, "bt_reset");
	if (rc)
		goto err_gpio_reset;
	rc = gpio_request(SUPERSONIC_GPIO_BT_SHUTDOWN_N, "bt_shutdown");
	if (rc)
...
err_rfkill_alloc:
	gpio_free(SUPERSONIC_GPIO_BT_SHUTDOWN_N);
err_gpio_shutdown:
	gpio_free(SUPERSONIC_GPIO_BT_RESET_N);
err_gpio_reset:
	return rc;
}
