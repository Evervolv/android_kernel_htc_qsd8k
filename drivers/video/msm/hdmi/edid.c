...
static struct video_mode established_timing_db[] = {
    ...
    {1024, 768, 87, ASPECT(4, 3), INTERLACE, false, "1024x768@87 Hz (Interlaced)"},
...
u8 edid_simple_parsing(struct hdmi_info *hdmi)
{
    ...
    for (extension = 0; extension < extensions; extension++)
    {
        ...
        if (dtdStart != 0)      dtdStart += baseOffset;
        ...
        for (i = 0; i < (edid_buf[baseOffset+3] & 0x0f); i++)   // Change by Ian: only bits 3-0 show length of blocks
        {
            unsigned blockType = edid_buf[baseOffset + dbcOffset] >> 5;
            unsigned blockLen = edid_buf[baseOffset + dbcOffset] & 0x1f;
            unsigned byte;

            // Skip any block which isn't video
            if (blockType == 3)
            {
                // We may have found the HDMI sink
                if (edid_buf[baseOffset + dbcOffset + 1] == 0x00 && 
                    edid_buf[baseOffset + dbcOffset + 2] == 0x0C && 
                    edid_buf[baseOffset + dbcOffset + 3] == 0x03 )
                {
                    // We found the HDMI block
                    edid_check_hdmi_sink(hdmi, i + 1);
                }
                continue;
            }
            if (blockType != 2)
